...
function mergeAudioFiles(_a) {
    return __awaiter(this, arguments, void 0, function (_b) {
        var inputPaths = _b.inputPaths, outputPath = _b.outputPath;
        return __generator(this, function (_c) {
            return [2 /*return*/, new Promise(function (resolve, reject) {
                    console.log('Starting audio merge process...');
                    console.log("Number of input files: ".concat(inputPaths.length));
                    ...
                    var command = ffmpeg();
                    // Add all input files
                    inputPaths.forEach(function (path, index) {
                        console.log("Adding input file ".concat(index + 1, ": ").concat(path));
                    ...
                    command
                        ...
                        console.log("Processing: ".concat(progress.percent ? progress.percent.toFixed(1) : 0, "% done"));
                    })
                        .on('error', function (err) {
                        console.error('Error during merge:', err.message);
                        reject(new Error("Error merging audio files: ".concat(err.message)));
                    })
                        .on('end', function () {
